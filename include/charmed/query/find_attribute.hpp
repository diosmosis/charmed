...
namespace charmed { namespace query
{
    ...
    template <typename M, int N, typename K>
    inline M const* find_attribute(K const& key)
    {
        typename metadata_iterator<M, N>::type i = metadata_storage<M>::metadata().get<N>().find(key);

        if (i == metadata_storage<M>::metadata().get<N>().end())
        {
            return 0;
        }
